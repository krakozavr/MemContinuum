...
| `newfile-nudge.sh` | `PreToolUse` (Write only, filtered to `--code-root`) | fires only when the write target does not exist yet and its extension is wired for this project; injects one reminder to search the code index first. Separately (newlang-nudge): when the extension is one the engine supports but this project never wired, injects a one-line nudge naming the language and pointing at the memcontinuum skill's re-wiring procedure -- deduped once per language per session, not scoped to `--code-root` |
