...
and the index filename. `--store DIR` is where the markdown store lives; omit it
and the conventional name is used — `<repo>-MemContinuum-Store` beside the repo
you are in. The store is always its own git repository, never a subdirectory of
the code it describes, and this command refuses to create one inside another
repo's working tree unless you pass `--force`. `--code-root DIR` (repeatable) is
the code checkout whose edits should trigger retrieval; omit it entirely for a
rationale-only install, and the two edit-time hooks are simply not wired.

Run it with `--dry-run` first — it prints every path and every hook line and
writes nothing. If you pass `--store`, pass `--claude-dir` too: an explicit
store can be wired from any directory, so the installer refuses to guess which
`.claude` the hooks belong in rather than wiring the wrong repo.

**The census, and being asked before anything is indexed.** With a
`--code-root`, the installer first counts what languages are actually in your
tree — by extension, and by reading the first line of extensionless files, so a
`#!/usr/bin/env python3` script with no `.py` suffix is counted as Python too —
and then asks what to do with what it found:
...
`--never-ext .cs` is the fourth answer's flag form. With no terminal and neither
flag, the installer exits with a message naming the driven flow instead of
hanging. Run `memidx.py code-census --root DIR` yourself any time to see the
same breakdown without wiring anything.
...
it is how a project ends up with a store that stopped being true.
...
Two things worth knowing: committing the store reindexes it automatically, so
`reindex` by hand is only for edits you have not committed yet; and
...
The code index handles **Swift and Python** today, both natively — Swift with a
tuned walker, Python with the standard library's own parser. More languages
arrive through tree-sitter in the next milestone; the registry they plug into is
already in place.
...
Files this engine cannot handle, and files whose language you did not enable,
are **reported, never skipped in silence**: every `code-reindex` ends with a
line naming each unindexed extension and how many files it passed over. A
growing blind spot that nobody can see is the failure mode this rule exists to
prevent.

If you pass several `--code-root` directories, the code index covers the first
one and the installer prints which roots it skipped. The other roots still get
the write-time reminder, but their code is not searchable yet; multi-root
